...
public class AdapterTransmissionImages extends RecyclerView.Adapter<AdapterTransmissionImages.RecyclerViewHolder> {
    ArrayList<PojoEngineImages> engineImages;
    Context context;
    ImageView car_eng_position;
    MediaPlayer mediaPlayer;

    public AdapterTransmissionImages(ArrayList<PojoEngineImages> engineImages, Context context) {
    ...
    @NotNull
    @Override
    public AdapterTransmissionImages.RecyclerViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.items_engine_images_lists, parent, false);
        return new AdapterTransmissionImages.RecyclerViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull AdapterTransmissionImages.RecyclerViewHolder holder, int position) {
        ...
        holder.rl_eng_images.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AddTransmissionImages.getInstance().selectedObject = position;
                if (engineImages.get(position).getImage() == null)
                {
                    AddTransmissionImages.getInstance().finalids.add(engineImages.get(position).getPart_id());
                    AddTransmissionImages.getInstance().CallCamera();
                }else{
                    AddTransmissionImages.getInstance().CallCamera();
                }
            }
